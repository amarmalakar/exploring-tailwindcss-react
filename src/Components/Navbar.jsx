...
const Navbar = () => {
    ...
    return (
        <Fragment>
            {/* large nav */}
            <nav className="hidden md:block bg-cyan-600 w-full">
            ...
            {/* small nav */}
            <nav className="md:hidden block bg-cyan-600 w-full">
                <div className="w-full p-2 flex flex-row justify-between items-center">
                    <Link to="/" className="text-xl font-semibold text-white ease-linear duration-150 hover:text-gray-900">Brand</Link>
                    <button onClick={mobSubMenuHandler} className="bg-cyan-500 shadow-md px-2 py-1 rounded text-white hover:bg-cyan-700 active:animate-ping">{isSubMenu ? 'Hide' : 'Show'}</button>
                </div>
                {isSubMenu && <ul className="text-white bg-cyan-700">
                    {links.map((link, idx) => (
                        <li className="hover:bg-cyan-900 hover:cursor-pointer" key={idx}>
